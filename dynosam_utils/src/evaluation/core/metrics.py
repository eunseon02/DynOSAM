...
class AME(APE):
    def __init__(self,
        ...
        super(APE, self).__init__(pose_relation)
...
class RME(APE):
    ...
    def process_data(self, data: TrajPair) -> None:
        ...
        object_traj_in_L = traj.PoseTrajectory3D(poses_se3=np.array(object_motion_L), timestamps=traj_motion_est.timestamps[1:])
        object_traj_in_L_ref = traj.PoseTrajectory3D(poses_se3=np.array(object_motion_L_gt), timestamps=traj_motion_est.timestamps[1:])
    ...
    def sync_object_motion_and_pose(data: TrajPair, max_diff = 0.01):
        ...
        if traj_pose_ref_sync.timestamps[0] >= traj_motion_est_sync.timestamps[0]:
            ...
            extra_pose_timestamp = traj_pose_ref.timestamps[extra_pose_index]
            extra_pose_pose = traj_pose_ref.poses_se3[extra_pose_index]
