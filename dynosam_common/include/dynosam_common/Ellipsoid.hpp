...
public:
    ...
    // Convert ellipsoid to 3D Gaussian representation
    std::tuple<Eigen::Vector3d, Eigen::Matrix3d> AsGaussian3D() const {
        if (has_changed_) {
            decompose();
            has_changed_ = false;
        }
        // Create diagonal matrix with squared axes
        Eigen::Matrix3d A_dual = Eigen::DiagonalMatrix<double, 3>(
            std::pow(axes_[0], 2),
            std::pow(axes_[1], 2),
            std::pow(axes_[2], 2)
        );
        // Rotate to get covariance matrix
        Eigen::Matrix3d cov = R_ * A_dual * R_.transpose();
        return {center_, cov};
    }
