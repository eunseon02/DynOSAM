...
ObjectMatcher::ObjectMatcher(Eigen::Matrix3d K) { K_ = K; }

ObjectMatcher::ObjectMatcher(const dyno::CameraParams& camera_params) {
  // Create camera intrinsic matrix K from CameraParams
  // Convert to the format expected by ObjectMatcher (Eigen::Matrix3d)
  cv::Mat K = cv::Mat::eye(3, 3, CV_32F);
  K.at<float>(0, 0) = static_cast<float>(camera_params.fx());
  K.at<float>(1, 1) = static_cast<float>(camera_params.fy());
  K.at<float>(0, 2) = static_cast<float>(camera_params.cu());
  K.at<float>(1, 2) = static_cast<float>(camera_params.cv());
  cv::Mat mK;
  K.copyTo(mK);
  cv::cv2eigen(mK, K_);
}

int ObjectMatcher::MatchObjectsHungarian(Frame& CurrentFrame,
...
//                 pMP->mnLastFrameSeen = CurrentFrame.getFrameId();
//             }
//         }
//     } */
...
//             nmatches += 1;
//             CurrentFrame.graph->attributes[node_id].obj = matched_obj;
//             //CurrentFrame.graph->attributes[node_id].obj->last_obs_frame_id_ = CurrentFrame.getFrameId();
//             CurrentFrame.graph->attributes[node_id].obj->last_obs_ids_and_max_iou.first =
//                 std::make_pair(CurrentFrame.getFrameId(), node_id);
//             CurrentFrame.graph->attributes[node_id].obj->last_obs_ids_and_max_iou.second = iou_max;
