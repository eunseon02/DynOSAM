...
namespace dyno 
{
    ...
    void Object::OptimizeReconstructionQuat(bool b_random_detections)
    {
        ...
        // Convert Rot3 to axis-angle representation (log map)
        // axisAngle() returns pair<Unit3, double>, extract axis and angle
        ...
        // Simple isotropic noise on Wasserstein distance
        ...
        for (auto i : chosen_indexes) {
            if (i >= observed_kfs.size() || i >= ellipses_.size()) {
