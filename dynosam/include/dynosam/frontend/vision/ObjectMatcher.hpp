...
class ObjectMatcher {
 public:
  using UniquePtr = std::unique_ptr<ObjectMatcher>;

  explicit ObjectMatcher(Eigen::Matrix3d K);
  explicit ObjectMatcher(const dyno::CameraParams& camera_params);
    ...
    int MatchObjectsWasserDistance(Frame &F, std::unordered_map<Object*, Ellipse> proj_bboxes, const Eigen::Matrix<double, 3, 4>& P);
