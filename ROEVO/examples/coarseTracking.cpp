...
int main(int argc, char **argv) {
    ...
    std::cout << "fx: " << ph.fx << ", fy: " << ph.fy << ", cx: " << ph.cx << ", cy: " << ph.cy << std::endl;
    ...
    for(int i = 0; i < N; ++i)
    {
        ...
        if(i == 0)
        {
            ...
            std::cout<<"selector.mvEdges.size(): "<<selector.mvEdges.size()<<std::endl;
        ...
        if(isKeyFrameChanged)
        {
            ...
            //-- 重置粗匹配的参考帧
            coarse_tracker.setReference(refMatGray, x_list, y_list, depth_list, weight_list, theta_list, pFrameRef->F_ID);
        ...
        if(isDataValid){
            auto start_timer = std::chrono::steady_clock::now();
            //-- 设置当前帧
            coarse_tracker.setCurrent(imgGray, i);
            //-- 并行计算当前帧到参考帧的位姿变换
            std::cout << "CoarseTrack: reference frame id=" << coarse_tracker.getReference()
                << ", current frame id=" << coarse_tracker.getCurrent() << std::endl;
            coarse_tracker.estimatePyramid(pose_gn, true);
            //-- TUM 형식으로 출력: timestamp tx ty tz qx qy qz qw
            Eigen::Vector3d t = pose_gn.translation();
            Eigen::Quaterniond q(pose_gn.rotationMatrix());
            std::cout << std::setprecision(20) << rgb_stamp_seq[i] << " "
                      << std::setprecision(6)
                      << t.x() << " " << t.y() << " " << t.z() << " "
