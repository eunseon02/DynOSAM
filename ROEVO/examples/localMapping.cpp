...
int main(int argc, char **argv) {
    ...
    //-- 日志输出间隔（프레임 수）
    const int log_interval = 10;  //-- 每10帧마다 로그 출력
    ...
    for(int i = 0; i < N; ++i)
    {
        //-- Debug: 각 반복마다 중단하여 확인
        // std::cout << "\033[36m[Debug] \033[0m" << "Frame " << i << "/" << N << " - Press Enter to continue..." << std::endl;
        // std::cin.get();
        
        //-- 得到当前帧的粗匹配位姿
        Eigen::Matrix4d pose_curr_coarse = tum_pose::getStaticGTPose(rgb_stamp_seq[i], coarse_stamps, coarse_poses);
        //-- 通过四元数归一化确保旋转矩阵的正交性（避免Sophus的严格正交性检查失败）
        ...
        if (select || i == 0)
        {
            ...
            size_t clusters_before = pLocalMap->mvEleEdgeClusters.size();
            size_t edges_before = 0;
            for(const auto& kf : pLocalMap->mvKeyFrames) {
                edges_before += kf->mvEdges.size();
            }
            size_t kf_count_before = pLocalMap->mvKeyFrames.size();
        
            pLocalMap->addFrame2LocalMap(pKF);
            
            //-- Log after adding keyframe
            size_t clusters_after = pLocalMap->mvEleEdgeClusters.size();
            size_t edges_after = 0;
            for(const auto& kf : pLocalMap->mvKeyFrames) {
                edges_after += kf->mvEdges.size();
            }
            size_t kf_count_after = pLocalMap->mvKeyFrames.size();
            
            // std::cout << "\033[32m[KF Add]\033[0m "
            //           << "kf_id=" << std::setw(4) << pKF->KF_ID
            //           << " | kf_count: " << std::setw(2) << kf_count_before << " -> " << std::setw(2) << kf_count_after
            //           << " | clusters: " << std::setw(4) << clusters_before << " -> " << std::setw(4) << clusters_after
            //           << " (+" << std::setw(3) << (clusters_after - clusters_before) << ")"
            //           << " | edges: " << std::setw(5) << edges_before << " -> " << std::setw(5) << edges_after
            //           << " (+" << std::setw(4) << (edges_after - edges_before) << ")"
            //           << std::endl;
            ...
            {
                LOG(INFO) << "Starting edge sliding-window optimization: kf_count=" << pLocalMap->mvKeyFrames.size()
                        << ", window_size=" << ph.win.window_size
                        << ", clusters_before=" << pLocalMap->mvEleEdgeClusters.size();
                ...
                //         << " / " << pLocalMap->mvEleEdgeClusters.size();
        ...
        // //-- 定期 로그 출력
        // if(i % log_interval == 0 || i == N - 1)
        // {
        //     std::cout << "\033[33m[Progress] \033[0m"
        //               << "Frame: " << i << "/" << N 
        //               << " (" << std::fixed << std::setprecision(1) << (100.0 * i / N) << "%)"
        //               << " | Process time: " << std::setprecision(2) << dt << " ms"
        //               << " | KeyFrames: " << pLocalMap->mvKeyFrames.size()
        //               << " | Clusters: " << clusterClouds.size()
        //               << std::endl;
        // }
