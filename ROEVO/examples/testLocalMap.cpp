...
// Sophus is strict about SO(3) orthogonality. Data / numeric noise can produce
// nearly-orthogonal matrices (1e-11 off) which will abort. Fix by projecting
// onto SO(3) via SVD.
static inline Eigen::Matrix3d OrthonormalizeRotation(const Eigen::Matrix3d& R_in) {
...
int main(int argc, char **argv) {
    ...
    for(int i = 0; i < clusterClouds.size(); ++i){
        ...
        *cloud_cluster = clusterClouds[i];
        ...
        // viz_util::drawPointCloudColor<pcl::PointXYZ>(viewer, id, cloud_cluster, color[0], color[1], color[2], 1);
        viz_util::drawPointCloudColor<pcl::PointXYZ>(viewer, id, cloud_cluster, 160, 160, 160, 1);
    }
    ...
    for(int i = 0; i < mergedCloud.size(); ++i){
        ...
        *cloud_cluster = mergedCloud[i];
        ...
        cv::Vec3b color = clusterCloudColors[i];
        // Generate a random (bright) color
        viz_util::drawPointCloudColor<pcl::PointXYZ>(viewer, id, cloud_cluster, 255, 0, 0, 4);
