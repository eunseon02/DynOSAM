    ...
    const float& fx, const float& fy, const float& cx, const float& cy)
{
    ...
    int edges_before_assignProperty3D = mvEdges.size();
    assignProperty3D(matDepth); //-- 为边缘赋值深度
    int edges_after_assignProperty3D = mvEdges.size();
    std::cout << "[ROEVO Frame] assignProperty3D: before=" << edges_before_assignProperty3D
              << ", after=" << edges_after_assignProperty3D << std::endl;
    
    // edgeCullingDepth();         //-- 剔除所有深度无效的边缘以及有效边缘中的无效边缘点
    int edges_before_culling = mvEdges.size();
    edgeCullingDepthParallel();
    int edges_after_culling = mvEdges.size();
    std::cout << "[ROEVO Frame] edgeCullingDepthParallel: before=" << edges_before_culling
              << ", after=" << edges_after_culling
              << ", removed=" << (edges_before_culling - edges_after_culling) << std::endl;
    
    //-- 现在剩下的edge中的所有edge point 都有有效深度
    int edges_before_continuity = mvEdges.size();
    edgeCullingContinuity();    //-- 确保每条有序边缘的3D点深度连续一致
    int edges_after_continuity = mvEdges.size();
    std::cout << "[ROEVO Frame] edgeCullingContinuity: before=" << edges_before_continuity
              << ", after=" << edges_after_continuity
              << ", removed=" << (edges_before_continuity - edges_after_continuity) << std::endl;
