...
{
    //-- Reset all clusters before re-constructing merged results
    for(size_t i = 0; i < mvEleEdgeClusters.size(); ++i)
    {
        elementEdgeCluster& cluster = mvEleEdgeClusters[i];
    ...
    //-- Iterate over all element-edge clusters
    for(size_t i = 0; i < mvEleEdgeClusters.size(); ++i)
    {
        ...
        //-- Collect all elementEdges and their owning keyframe indices for this cluster
        int N = ele_indices.size();
        for(size_t j = 0; j < N; ++j)
        {
            elementEdge& ele_edge = mvElementEdges.at(ele_indices[j]);
        ...
        //-- Merged edge cloud expressed in the local map reference frame
        std::vector<cv::Point3d> merged_cloud;
        double dist_thres;
        std::vector<int> involved_elements;
        //-- Merge all elementEdges within this cluster into a single representative edge
        //-- (projection-based / iterative fitting in the reference frame)
        featureMerger::getMergedClusterIterative(clusterElement, kf_indices, mvKeyFrames, merged_cloud, dist_thres, involved_elements);
        //-- Store merged result back into the cluster
